...
class YOLOX(nn.Module):
    def __init__(self, opt, backbone, neck, head, loss):
        ...
        self.lwin = nn.Parameter(torch.tensor(0.1, requires_grad=True), requires_grad=True)
        self.rwin = nn.Parameter(torch.tensor(0.9, requires_grad=True), requires_grad=True)

    def forward(self, inputs, targets=None, show_time=False):
        with torch.cuda.amp.autocast(enabled=self.opt.use_amp):
            if show_time:
                s1 = sync_time(inputs)
            #preprocess trainable parameter(window center, window width)
            #inputs[np.where(inputs < self.lwin)] = self.lwin
            #inputs[np.where(inputs > self.rwin)] = self.rwin
            inputs =  torch.minimum(torch.maximum(self.lwin,inputs),self.rwin)
            inputs = inputs / (self.rwin - self.lwin)
