...
def main(config):
    ...
    filename = "./valid_mean_results_0509/result_0509.csv"

    f = open(filename, 'w', newline='')
    wr = csv.writer(f)
    wr.writerow(['frame_index','predtict' , 'label' ])

    filename_new = "./valid_mean_results_0509/resultforMean_0509.csv"

    f_n = open(filename_new, 'w', newline='')
    wr_n = csv.writer(f_n)
    wr_n.writerow(["name","index", "0_mean" , "1_mean" , "2_mean" , "label", "predict"])
    ...
    with torch.no_grad():
        for root, _, fnames in sorted(os.walk(config['video_path'], followlinks=True)):
            ...
            for num, fname in enumerate(path_list):
                ...
                y_label = 0
                if os.path.basename(os.path.dirname(path)) == "stomach":
                    y_label = 0
                elif os.path.basename(os.path.dirname(path)) == "small_colon":
                    y_label = 1
                else:
                   y_label = 2
                ...
                while True:
                    ...
                    if len(pred_prob) == config['clip_num']:
                        ...
                        pred_mean_array = np.mean(pred_prob, axis = 0)       
                        ...
                        result_list = []
                        result_list.append(fname.split('.')[0])
